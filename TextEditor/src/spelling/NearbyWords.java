...
//		          add n to the list of words to return
		...
		while(!queue.isEmpty() && numSuggestions > 0 && visited.size() < THRESHOLD) {
			curr = queue.remove(0);
			List<String> wordList = distanceOne(curr, true);
			for(String wordOneAway : wordList) {
				if(!visited.contains(wordOneAway)) {
					visited.add(wordOneAway);
					queue.add(wordOneAway);
					if(dict.isWord(wordOneAway)) {
						retList.add(wordOneAway);
						numSuggestions --;
					}	
				}
